...
class BostonDormDashboard:
    ...
    def _create_widgets(self):
        ...
        # Filter checkboxes
        self.show_dorms = pn.widgets.Checkbox(
            name="Show Northeastern Dorms", value=True
        )
        self.show_grocery = pn.widgets.Checkbox(name="Show Grocery Stores", value=True)
        self.show_trader_joes = pn.widgets.Checkbox(
            name="Show Trader Joe's", value=True
        )
        self.show_convenience = pn.widgets.Checkbox(
            name="Show Convenience/Pharmacy", value=False
        )
        ...
        self.custom_name = pn.widgets.TextInput(
            name="Location Name", placeholder="e.g., My Apartment"
        )
        self.custom_lat = pn.widgets.FloatInput(
            name="Latitude", value=42.3398, step=0.0001, start=42.0, end=43.0
        )
        self.custom_lng = pn.widgets.FloatInput(
            name="Longitude", value=-71.0892, step=0.0001, start=-72.0, end=-70.0
        )
        self.add_custom_btn = pn.widgets.Button(
            name="Add Custom Location", button_type="primary"
        )
        self.clear_custom_btn = pn.widgets.Button(
            name="Clear Custom Location", button_type="warning"
        )
        ...
        self.radius_filter = pn.widgets.FloatSlider(
            name="Search Radius (miles)", start=0.5, end=5.0, step=0.5, value=2.0
        )

        # Update button
        self.update_btn = pn.widgets.Button(
            name="Update Map", button_type="success", width=200
        )
    ...
    def _add_dorms_layer(self, m):
        ...
        for dorm in dorms:
            ...
            folium.Marker(
                ...
                tooltip=dorm["name"],
                icon=folium.Icon(
                    color=self.colors["dorm"], icon=self.icons["dorm"], prefix="fa"
                ),
            ).add_to(dorms_group)
    ...
    def _add_custom_location_layer(self, m):
        ...
        folium.Marker(
            ...
            icon=folium.Icon(
                color=self.colors["custom"], icon=self.icons["custom"], prefix="fa"
            ),
        ).add_to(custom_group)
    ...
    def create_dashboard(self):
        ...
        dashboard = pn.template.FastListTemplate(
            ...
            sidebar=[sidebar],
            main=[title, info, stats, self.map_pane],
