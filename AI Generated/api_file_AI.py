...
class BostonDataAPI:
    ...
    def load_data(self):
        """Load all CSV files"""
        try:
            self.dorms_df = pd.read_csv(
                "../data/NortheasternDorm_data/layers/dorms_with_prices.csv"
            )
            ...
            self.mbta_nodes_df = pd.read_csv(
                "../data/MBTA_data/MBTA Rapid Transit Labels/mbta_nodes.csv"
            )
            self.mbta_lines_df = pd.read_csv(
                "../data/MBTA_data/MBTA Rapid Transit Lines/mbta_lines.csv"
            )
    ...
    def find_closest_locations(
        self, lat: float, lng: float, locations: List[Dict], n: int = 3
    ) -> List[Dict]:
    ...
    def analyze_location(self, name: str, lat: float, lng: float) -> Dict:
        ...
        if groceries:
            results["closest_grocery"] = self.find_closest_locations(
                lat, lng, groceries, 3
            )
        if trader_joes:
            results["closest_trader_joes"] = self.find_closest_locations(
                lat, lng, trader_joes, 3
            )
        if conv_pharm:
            results["closest_convenience_pharmacy"] = self.find_closest_locations(
                lat, lng, conv_pharm, 3
            )
        if mbta:
